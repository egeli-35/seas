...
module.exports = {
    ...
    async execute(client,message,args){
      ...
      const emojiler = {
        ...
        const erkek = message.guild.roles.cache.find(r => r.id === "794958111735021628")
        const xy = message.guild.roles.cache.find(r => r.id === "794958112322093057")
      ...
      if(member.id === message.author.id) return message.channel.send(chery10 = new Discord.MessageEmbed().setAuthor(message.author.tag,message.author.displayAvatarURL({dynamic:true})).setDescription("`Kendini Kayıt Edemesin`").setColor("RED")).then(message.react(emojiler.tehdit)).then(x => x.delete({timeout:9000}))
        ...
        .setColor("RED")
        .setDescription(`${member} Kullanıcısına <@&794958111735021628> Rolleri Verildi `)
        .setFooter(`Toplam Kayıtların ${toplami} `)
       reglog.send(chery)
